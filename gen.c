...
static int genIFAST(struct ASTnode *n) {
    ...
    // Generate two labels: for the false compound statement and the end of the overall IF statement.
    ...
    // Send the `Lfalse` label as a register.
    ...
    // If there is an `else`, generate the jump to skip to the end.
    if (n->right) {
        cgjump(Lend);
    }
